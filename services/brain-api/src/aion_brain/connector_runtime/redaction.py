"""Redaction helpers for disabled connector-runtime preview payloads."""
...
def redact_connector_runtime_payload(
    value: Any,
    *,
    findings: list[dict[str, object]] | None = None,
) -> Any:
    """Return a safe copy of connector-runtime preview input."""
    ...
    if isinstance(value, dict):
        ...
        for key, nested in value.items():
            ...
            if finding is not None:
                ...
                continue
            redacted[str(key)] = redact_connector_runtime_payload(nested, findings=findings)
        return redacted
    if isinstance(value, list):
        return [redact_connector_runtime_payload(item, findings=findings) for item in value]
...
def payload_findings(value: Any) -> list[dict[str, object]]:
    """Return unsafe connector-runtime payload findings without exposing values."""

    findings: list[dict[str, object]] = []
    redact_connector_runtime_payload(value, findings=findings)
    return findings


def connector_runtime_payload_has_unsafe_content(value: Any) -> bool:
    """Return whether a payload contains connector credentials or unsafe content."""
