"""AION-148 v0.2 implementation authorization stabilization tests."""
...
TRUE_KEYS = {
    "v02_implementation_authorization_stabilized",
    "implementation_authorization_stabilized",
...
def test_v02_implementation_authorization_stabilization_docs_exist() -> None:
    for relative in DOCS:
        assert (ROOT / relative).exists(), relative
    assert "0139-v02-implementation-authorization-stabilization.md" in _text(
        "docs/adr/README.md"
    )

    gate = _text("docs/release/v02-implementation-authorization-stabilization-gate.md")
    for required in [
        "## Purpose",
        "## Scope",
        ...
        "## No-go conditions",
        "AION-148 creates no v0.2 tag and no v0.2 release.",
    ]:
        assert required in gate

    freeze = _text("docs/release/v02-explicit-approval-record-freeze.md")
    for required in [
    ...
    baseline = _text("docs/release/v02-runtime-enablement-guard-evidence-baseline.md")
    for required in [
...
def test_v02_implementation_authorization_scripts_are_executable_and_pass() -> None:
    env = os.environ.copy()
