...
def test_expiry_plan_marks_prior_version_reference_only():
    seed = sample_planning_components(transaction_id="promotion-transaction-expiry-seed")
    ...
    plan = glm.plan_knowledge_version(
        ...
        request_kind=glm.PromotionRequestKind.EXPIRY,
        conflict_report=conflict_report,
        existing_references=(existing,),
        effective_from=seed.context.request.requested_at,
    )

    assert plan.expires_version_id == existing.reference_id
