...
def test_aion155_required_files_exist_and_readme_state_is_current() -> None:
    ...
    assert "stabilized internal disabled production-auth core" in normalized
    assert "AION-157-PA-0004" in readme
...
def test_aion155_json_is_synthetic_read_only_and_exactly_scoped() -> None:
    ...
    for relative in ACTIVE_JSON_ARTIFACTS:
        ...
        assert (
            ...
            == "2fbeb77bdc33772c46a679cbfa0bdafe327abb42"
        )
        assert (
            payload["authorization_consumed_by_merge_commit"]
            == "051f6f2e8b901863f8dc9cad405e5b5401db3695"
        )
