...
def test_semantic_projection_is_planned_without_memory_write():
    components = sample_planning_components(
        targets=(glm.MemoryProjectionTarget.SEMANTIC_MEMORY,),
    )
    record = components.projections.records[0]

    assert record.target is glm.MemoryProjectionTarget.SEMANTIC_MEMORY
