...
def test_aion157_required_files_exist_and_status_is_current() -> None:
    ...
    assert "Current milestone: AION-156 request-identity boundary implemented and merged." in status
    assert "Current authorization: AION-157-PA-0004 active for AION-158." in status
    assert "Next task: AION-158 request-identity boundary stabilization." in status
    ...
    assert "`AION-158` is the next critical path" in readiness
...
def test_aion157_validator_accepts_exact_four_record_lifecycle() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda records: records[3][1].__setitem__("candidate_id", "production-auth-runtime"),
            "candidate_id mismatch",
        ),
        (
            lambda records: records[3][1].__setitem__("workstream", "production-auth-runtime"),
            "workstream mismatch",
        ),
        (
            lambda records: records[3][1].__setitem__(
        ...
        (
            ...
            "pure_asgi_middleware_migration_approved must be true",
        ),
        (
            lambda records: records[3][1].__setitem__("extra_runtime_permission_approved", True),
        ...
        (
            lambda records: records[3][1].__setitem__("authenticated_requests_enabled", True),
        ...
        (
            lambda records: records[3][1].__setitem__("cookie_parsing_approved", True),
        ...
        (
            lambda records: records[3][1].__setitem__("login_endpoint_approved", True),
            "login_endpoint_approved must be false",
        ),
        (
            lambda records: records[3][1].__setitem__("openapi_security_scheme_added", True),
