...
def test_aion228_source_scope_is_recorded_and_exactly_implemented() -> None:
    ...
    source_scope = (
        auth.get("future_authorized_source_scope")
        or auth.get("historical_future_authorized_source_scope")
        or auth.get("implemented_source_scope")
    )
    assert tuple(source_scope[:-1]) == auth227.FUTURE_AION228_SOURCE_SCOPE
