...
def test_project_status_reports_aion180_current_state() -> None:
    ...
    assert "AION-218 verified-knowledge memory operator evaluation" in current
    assert "`active_knowledge_implementation_task=AION-219`" in current
    ...
    assert "`active_knowledge_implementation_task=AION-217`" in historical
