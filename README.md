...
## AION-138 v0.2 Decision Package Preview

AION-138 packages the stabilized v0.2 review, routing, submission, request,
proposal, queue, and runtime-boundary evidence into an approval-readiness
preview. The decision package is preview-only: decision package approval
remains false, approval readiness approved remains false, review board
decision approval remains false, routing decision approval remains false,
reviewer sign-off implementation approval remains false, submission approval
remains false, request pack approval remains false, runtime implementation
approval remains false, no external calls are added, no credentials or tokens
are stored, no sandbox execution is enabled, no v0.2 tag is created, and no
v0.2 release is created.
...
## AION-142 v0.2 Approval Docket Stabilization

AION-142 stabilizes the v0.2 approval docket layer and freezes implementation
decision records as unapproved planning evidence. The stabilization remains
preview-only: approval docket stabilization approval remains false, approval
docket item approval remains false, implementation decision record freeze
approval remains false, implementation decision record approval remains false,
runtime approval review approval remains false, runtime decision lock release
approval remains false, decision package approval remains false, approval
readiness approval remains false, review board decision approval remains false,
routing decision approval remains false, reviewer sign-off implementation
approval remains false, runtime implementation approval remains false, no
external calls are added, no credentials or tokens are stored, no sandbox
execution is enabled, no v0.2 tag is created, and no v0.2 release is created.
...
## AION-222 governed learning memory promotion planning
...
## AION-234 Secure Runtime Integration Status

AION-234 completed `AION-SRIPE-002` with decision `CONTROLLED_PROVIDER_NEUTRAL_MODEL_GATEWAY_OPERATOR_EVALUATION_PASS_RECOMMEND_SANDBOXED_CAPABILITY_RUNTIME_AUTHORIZATION`. The controlled provider-neutral model gateway passed operator evaluation. `AION-232-SRI-0002` is closed, consumed by AION-233, expired, and non-reusable. `AION-234-SRI-0003` is now the sole active Secure Runtime Integration implementation authorization for AION-235.

AION-235 is authorized to implement a sandboxed deterministic capability and synthetic connector runtime, but no AION-235 runtime source exists yet. Model output remains untrusted and cannot trigger execution. Explicit operator capability selection remains mandatory. External connectors, real tools, networks, credentials, filesystems, processes, shells, subprocesses, browsers, dynamic imports, eval, exec, package installation, modules, production memory, production policy, beliefs, source rewrite, deployment, and model training remain disabled. AION-236 is the next formal evaluation task. AION-237 remains unauthorized. AION-238 remains the final Secure Runtime Integration Program evaluation and v0.2 release-candidate review. v0.2 remains unreleased.
...
AION-244 records a PASS final evaluation for `aion-v0.2.0-rc.1` and authorizes only a GitHub prerelease publication for that RC1 candidate. Stable v0.2.0 tags, stable releases, production deployment, registry push, and public package upload remain prohibited until a separate stable-release evaluation exists.
