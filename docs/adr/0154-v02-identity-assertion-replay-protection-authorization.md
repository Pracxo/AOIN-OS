...
AION-164 can implement the internal persistent replay-protection core with existing dependencies and no migration. Request-level verified identity integration remains blocked until replay protection, schema provisioning, operational key provisioning, and a separate runtime integration authorization are complete.
