...
## AION-164 Replay Gate

The replay gate is `scripts/production-auth-identity-assertion-replay-check.sh`. It verifies the persistent replay core, exact table contract, focused replay tests, no-go regression, runtime hold, docs, and inherited offline assertion gates while preserving the production-auth runtime hold.
