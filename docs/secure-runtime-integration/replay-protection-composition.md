...
Replay protection uses the existing persistent replay pipeline through injection. Tests and the pilot use only an in-memory SQLite engine and auto-created existing metadata; no migration or replay database file is added.
