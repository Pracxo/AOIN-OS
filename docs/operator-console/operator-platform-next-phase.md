...
AION-139 adds read-only static console panels for decision package
stabilization and approval readiness freeze. The panels expose bundled,
redacted JSON evidence only and no inputs, write controls, approval controls,
...
The operator console next phase may show AION-149 final authorization evidence
as bundled, read-only JSON. It must not expose approval controls, write
