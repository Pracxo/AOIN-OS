...
AION-246 may define provider-neutral external-cognition contracts, manifests,
request and response envelopes, routing policies, budgets, structured-output
validation, trust, redaction, deterministic fixtures, replay, circuit-breaker,
audit and evidence controls. It may not call a live provider, use the network,
read credentials, write persistent memory, execute tools, execute connectors,
or enable production runtime.
...
## AION-133 Contract Boundary

AION-133 adds no runtime Brain contract. It records the request pack final
review, evidence boundary closeout, pre-approval submission gate, request
approval guard, final submission evidence matrix, final checklist, and no-go
evidence as static planning/request artifacts. Contract resources remain
unchanged, proposal registry and approval queue data remain preview-only, and
request pack approval, submission approval, preapproval gate bypass, runtime
implementation approval, proposal implementation approval, approval queue item
approval, external calls, credentials/tokens, sandbox execution, operator write
execution, connector runtime, production auth, and module activation remain
false or absent.
...
## AION-244 v0.2.0-rc.1 Contract Boundary

The final evaluation contract fixes `aion-v0.2.0-rc.1`, package version `0.2.0rc1`, source commit `d35f1caa234d35dce1dfc0a80bc4c8e327a8373e`, 32 hard-gated scenarios, and 24 release assets. Any stable release requires a separate contract and candidate build.
