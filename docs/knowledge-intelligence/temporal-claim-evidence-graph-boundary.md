...
# Temporal Claim-Evidence Graph Boundary

AION-209 may accept explicit structured unverified claim assertions and bind them to AION-207 source-registry records, citations, provenance, and lineage groups. Source registry references are required, and citation binding is required for evidence edges.

Automatic claim extraction, source-body parsing, raw source content storage, claim verification, truth decision, epistemic confidence, contradiction resolution, automatic acceptance or rejection, knowledge promotion, cognitive belief creation, cognitive belief mutation, persistent graph writes, graph databases, network acquisition, provider calls, connector calls, source mutation, Git mutation, runtime PRs, approvals, automatic merge, deployment, and model training remain prohibited.

The authorized scope is `append-only-immutable-temporal-claim-evidence-provenance-jurisdiction-version-contradiction-graph-core`.
