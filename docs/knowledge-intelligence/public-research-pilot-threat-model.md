# Public Research Pilot Threat Model
...
Threat controls cover SSRF, DNS rebinding, TOCTOU destination changes, IPv4-mapped IPv6 bypass, ambiguous IP encoding, metadata-service access, private-network access, redirect downgrade, certificate mismatch, malicious proxy inheritance, credential leakage, cookie leakage, compressed responses, slow responses, MIME confusion, prompt injection, robots-policy violation, duplicate-source amplification, citation spoofing, confidence amplification, stale evidence, retraction suppression, dissent suppression, tool output treated as fact, engagement poisoning, persistent source storage, background crawling, provider expansion, source mutation, Git mutation, deployment, authorization reuse, and evaluation evidence used as approval.
