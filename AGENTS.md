...
- Secure Runtime Integration Program implementation work for AION-231 may use
  only the exact secure-runtime foundation source scope, docs, examples, static
  console evidence, tests, an uninstalled local runner, and local gates. It
  must preserve completed Cognitive Architecture, Knowledge Intelligence,
  Governed Learning and Memory, and self-improvement program states. It must
  not create public auth endpoints, store credentials or tokens, issue session
  tokens, call external identity providers, call model providers, call
  connectors, execute tools, execute shells or subprocesses, automate browsers,
  activate modules, register dynamic routes, create schedulers or workers,
  mutate production memory, mutate production policy, mutate beliefs, mutate
  source, mutate Git, deploy, train model weights, create a v0.2 tag, or
...
- AION-232 evaluation closeout work records `AION-SRIPE-001`, closes `AION-230-SRI-0001`, and creates `AION-232-SRI-0002` for AION-233 only.
- AION-233 implements the controlled provider-neutral model gateway as local deterministic reference simulation only. It must remain credential-free, network-disabled, connector-disabled, tool-disabled, prompt-persistence-disabled, response-persistence-disabled, memory-write-disabled, policy-mutation-disabled, belief-mutation-disabled, deployment-disabled, and model-training-disabled until a later explicit authorization changes that boundary. `AION-232-SRI-0002` remains active pending AION-234 closeout.

- AION-234 closeout work records `AION-SRIPE-002`, closes `AION-232-SRI-0002`, and creates `AION-234-SRI-0003` for future AION-235 only. AION-235 may implement only closed, schema-validated, deterministic, in-memory reference operations through explicit operator selection, policy, risk, guardrails, existing approval evidence, zero-external-effect budgets, and kill-switch controls. It must not execute external connectors, real tools, networks, filesystems, processes, shells, subprocesses, browsers, dynamic imports, eval, exec, package installation, production memory, production policy, beliefs, source rewrite, deployment, model training, or v0.2 release actions.
...
For v0.2 release qualification work, `AION-244-V02REL-0001` is single-use and scoped only to the annotated `aion-v0.2.0-rc.1` tag at `d35f1caa234d35dce1dfc0a80bc4c8e327a8373e`, one GitHub prerelease, 24 candidate asset uploads, and post-publication reconciliation. Do not create stable v0.2 tags or production deployments from this authorization.
