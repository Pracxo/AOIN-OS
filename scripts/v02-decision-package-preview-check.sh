...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_DECISION_PACKAGE_PREVIEW_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-138 decision package artifact: $file" >&2
    exit 1
  }
done

grep -q "0129-v02-decision-package-preview.md" docs/adr/README.md || {
  echo "ADR 0129 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 decision package inherited gates deferred to outer aggregate gate"
else
...
if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-138" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-decision-package-preview.json",
    root / "examples/release/v02-approval-readiness-evidence-bundle.json",
    root / "examples/release/v02-runtime-decision-boundary.json",
    root / "examples/release/v02-decision-package-state-model.json",
    root / "examples/release/v02-decision-package-evidence-matrix.json",
    root / "operator-console-static/demo-data/v02-decision-package-preview.json",
    root / "operator-console-static/demo-data/v02-approval-readiness-evidence-bundle.json",
]
true_keys = {
...
for path in paths:
    ...
    if payload.get("task_id") != "AION-138":
        raise SystemExit(f"{path} must identify AION-138")
...
print("AION-138 JSON examples valid")
PY

cat <<'SUMMARY'
v0.2 decision package preview check result:
- v02_decision_package_preview_created: true
...
v0.2 decision package preview check PASS
SUMMARY
