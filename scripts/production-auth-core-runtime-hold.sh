...
is_nested_gate_context() {
  ...
  [[ "${AION_PRODUCTION_AUTH_CORE_RUNTIME_HOLD_SKIP_FULL_CHECK:-}" == "1" ]] && return 0
...
payload = json.loads(
    (root / "operator-console-static/demo-data/production-auth-runtime-hold.json").read_text()
)
required_false = [
