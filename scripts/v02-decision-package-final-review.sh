...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_DECISION_PACKAGE_FINAL_REVIEW_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-140 decision package final review artifact: $file" >&2
    exit 1
  }
done

grep -q "0131-v02-decision-package-final-review.md" docs/adr/README.md || {
  echo "ADR 0131 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 decision package final review inherited gates deferred to outer aggregate gate"
else
...
if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-140" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-decision-package-final-review.json",
    root / "examples/release/v02-approval-readiness-closeout.json",
    root / "examples/release/v02-runtime-decision-lock.json",
    root / "examples/release/v02-decision-package-final-evidence-matrix.json",
    root / "examples/release/v02-decision-package-approval-guard.json",
    root / "operator-console-static/demo-data/v02-decision-package-final-review.json",
    root / "operator-console-static/demo-data/v02-runtime-decision-lock.json",
]
true_keys = {
...
for path in paths:
    ...
    assert payload["task_id"] == "AION-140", path
...
print("AION-140 JSON examples valid")
PY

cat <<'SUMMARY'
v0.2 decision package final review result:
- v02_decision_package_final_review_passed: true
...
v0.2 decision package final review PASS
SUMMARY
