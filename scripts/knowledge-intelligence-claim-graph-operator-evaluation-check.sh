...
program, auth, closed_record, active_record = assert_ledgers()
assert report["authorization_closeout"]["authorization_transaction_id"] == CLAIM_AUTH
...
import json
import os
required = {"brain-api-quality", "contract-check", "docker-build-core", "policy-check", "repository-hygiene", "sdk-cli-check", "sdk-quality"}
pr = json.loads(os.environ["PR_JSON"])
checks = json.loads(os.environ["PR_CHECKS_JSON"])
assert pr["number"] == 121
assert pr["state"] == "MERGED"
assert pr["baseRefName"] == "main"
assert pr["headRefName"] == "phase/knowledge-intelligence-temporal-claim-evidence-graph"
assert pr["headRefOid"] == "d50252c84a0a02b75317c7d2051eaee4fb9dc54c"
assert pr["mergeCommit"]["oid"] == "f9e2438a49aae458983fc57cee5c12b5ef0ab856"
assert pr["mergedAt"] == "2026-07-24T10:13:38Z"
