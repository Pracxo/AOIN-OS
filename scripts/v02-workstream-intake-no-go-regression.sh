...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-124 workstream intake artifact: $file" >&2
    exit 1
  }
done

if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-124" >&2
  exit 1
fi

if rg -n 'gh[[:space:]]+release[[:space:]]+create[[:space:]]+|release[[:space:]]+created[[:space:]]*[:=][[:space:]]*true|v02_release_created[[:space:]]*[:=][[:space:]]*true' \
  docs/release docs/adr/0115-v02-workstream-intake-readiness.md examples/release operator-console-static/demo-data; then
...
if rg -n '\b(runtime_implementation_approved|operator_write_execution_approved|connector_implementation_approved|production_auth_approved|module_activation_approved|external_calls_approved|credential_storage_approved|token_storage_approved|sandbox_execution_approved|oauth_oidc_saml_runtime_approved|code_loading_approved|runtime_registration_approved|capability_activation_approved|v02_release_approved|v02_release_created|package_files_added|migrations_added|api_runtime_execution_route_added|sdk_resource_implementation_added|cli_command_implementation_added|frontend_dependencies_added|backlog_implementation_items_approved|workstream_implementation_approved|approval_workflow_bypassed|approval_record_missing|adr_dependency_bypassed|gate_dependency_bypassed|approval_expiry_bypassed|approval_revocation_bypassed|dual_control_bypassed)\s*[:=]\s*true\b' \
  "${scan_paths[@]}"; then
  echo "implementation approval, missing approval record, bypass, or drift boolean was enabled" >&2
  exit 1
fi
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-124 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-124 examples or static data" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-workstream-intake-readiness-gate.json",
    root / "examples/release/v02-workstream-intake-evidence-pack.json",
    root / "examples/release/v02-approval-record-evidence-pack.json",
    root / "examples/release/v02-implementation-sequencing-freeze.json",
    root / "examples/release/v02-workstream-readiness-scorecard.json",
    root / "operator-console-static/demo-data/v02-workstream-intake-readiness.json",
    root / "operator-console-static/demo-data/v02-implementation-sequencing-freeze.json",
]
false_keys = {
...
for path in paths:
    ...
    if relative.startswith("examples/") and payload.get("synthetic") is not True:
        raise SystemExit(f"workstream intake example must be synthetic: {path}")
    if payload.get("status") != "passed":
        raise SystemExit(f"workstream intake artifact must be passed: {path}")
    if payload.get("task_id") != "AION-124":
        raise SystemExit(f"workstream intake task id must be AION-124: {path}")
    if payload.get("v02_workstream_intake_ready") is not True:
        raise SystemExit(f"workstream intake ready flag must be true: {path}")
    walk(payload, relative)

print("v0.2 workstream intake no-go JSON checks PASS")
PY

cat <<'SUMMARY'
v0.2 workstream intake no-go regression result:
...
- migrations: absent
v0.2 workstream intake no-go regression PASS
SUMMARY
