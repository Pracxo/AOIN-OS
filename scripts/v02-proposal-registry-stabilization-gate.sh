...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_PROPOSAL_REGISTRY_STABILIZATION_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-127 proposal registry stabilization artifact: $file" >&2
    exit 1
  }
done

grep -q "0118-v02-proposal-registry-stabilization.md" docs/adr/README.md || {
  echo "ADR 0118 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 proposal registry stabilization inherited gates deferred to outer aggregate gate"
else
  run_inherited_gate ./scripts/v02-workstream-proposal-registry-check.sh
  run_inherited_gate ./scripts/v02-proposal-registry-freeze.sh
  run_inherited_gate ./scripts/v02-proposal-registry-no-go-regression.sh
...
if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-127" >&2
  exit 1
fi
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-127 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-127 examples or static data" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-proposal-registry-stabilization-gate.json",
    root / "examples/release/v02-approval-queue-freeze.json",
    root / "examples/release/v02-candidate-workstream-evidence-baseline.json",
    root / "examples/release/v02-proposal-lifecycle-evidence-matrix.json",
    root / "examples/release/v02-proposal-registry-closeout-result.json",
    root / "operator-console-static/demo-data/v02-proposal-registry-stabilization.json",
    root / "operator-console-static/demo-data/v02-approval-queue-freeze.json",
]
false_keys = {
...
def walk(value, context):
    if isinstance(value, dict):
        for key, nested in value.items():
            ...
            if key in {"implementation_approved", "approval_status", "implementation_status", "runtime_enabled"} and nested is not False:
                raise SystemExit(f"{context}.{key} must be false")
...
for path in paths:
    ...
    if payload.get("task_id") != "AION-127":
        raise SystemExit(f"{path} has wrong task_id")
    if payload.get("status") != "passed":
        raise SystemExit(f"{path} has wrong status")
    if payload.get("synthetic") is not True:
        raise SystemExit(f"{path} must be synthetic")
    if payload.get("v02_proposal_registry_stabilized") is not True:
        raise SystemExit(f"{path} must mark registry stabilized")
    if payload.get("proposal_registry_preview_only") is not True:
        raise SystemExit(f"{path} must be registry preview-only")
    if payload.get("approval_queue_preview_only") is not True:
        raise SystemExit(f"{path} must be approval queue preview-only")
    walk(payload, str(path))
PY

cat <<'SUMMARY'
v0.2 proposal registry stabilization gate result:
- v02_proposal_registry_stabilized: true
...
v0.2 proposal registry stabilization gate PASS
SUMMARY
