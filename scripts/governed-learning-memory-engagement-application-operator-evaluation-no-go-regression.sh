...
if base="$(comparison_base "${1:-}")"; then
  ...
  if printf '%s\n' "$changed" | awk '$1 ~ /^D|R/ {print}' | rg -n '.+'; then
    echo "ERROR: AION-227 primary branch deletes or renames existing files" >&2
    exit 1
  fi
  if printf '%s\n' "$changed" | awk '{for (i=2; i<=NF; i++) print $i}' | rg -n '^services/brain-api/src/aion_brain/'; then
    echo "ERROR: AION-227 primary branch must not modify runtime source" >&2
    exit 1
  fi
...
if git ls-files | rg -n 'services/brain-api/src/aion_brain/(contracts/governed_continual_learning\.py|governed_learning_memory/continual_learning_)'; then
...
if rg -n \
  ...
  '(network_calls": [1-9]|dns_resolutions": [1-9]|connector_calls": [1-9]|model_provider_calls": [1-9]|source_mutations": [1-9]|git_operations": [1-9]|v02_release_ready": true|production_exposure": true)' \
  docs/governed-learning-memory docs/release examples/governed-learning-memory operator-console-static/demo-data; then
  echo "ERROR: AION-227 no-go boundary violated" >&2
  exit 1
fi
...
echo "governed learning memory engagement application operator evaluation no-go PASS"
