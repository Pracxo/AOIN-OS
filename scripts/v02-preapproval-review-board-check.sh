...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_PREAPPROVAL_REVIEW_BOARD_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-136 pre-approval review board artifact: $file" >&2
    exit 1
  }
done

grep -q "0127-v02-preapproval-review-board.md" docs/adr/README.md || {
  echo "ADR 0127 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 pre-approval review board inherited gates deferred to outer aggregate gate"
else
...
if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-136" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-preapproval-review-board.json",
    root / "examples/release/v02-submission-review-routing.json",
    root / "examples/release/v02-reviewer-role-matrix.json",
    root / "examples/release/v02-decision-readiness-boundary.json",
    root / "examples/release/v02-review-board-evidence-pack.json",
    root / "operator-console-static/demo-data/v02-preapproval-review-board.json",
    root / "operator-console-static/demo-data/v02-submission-review-routing.json",
]
true_keys = {
...
for path in paths:
    ...
    if payload.get("task_id") != "AION-136":
        raise SystemExit(f"{path} must identify AION-136")
...
routing = json.loads((root / "examples/release/v02-submission-review-routing.json").read_text())
...
print("AION-136 JSON examples valid")
PY

cat <<'SUMMARY'
v0.2 pre-approval review board check result:
- v02_preapproval_review_board_created: true
...
v0.2 pre-approval review board check PASS
SUMMARY
