...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_WORKSTREAM_INTAKE_FREEZE_SKIP_FULL_CHECK:-}" == "1" ]] && return 0
...
if is_nested_gate_context; then
  echo "PASS: v0.2 workstream intake gate deferred to outer aggregate gate"
else
  AION_AGGREGATE_GATE_RUNNING=1 ./scripts/v02-workstream-intake-readiness-gate.sh
fi
...
echo "v0.2 workstream intake freeze working tree summary:"
...
cat <<SUMMARY
v0.2 workstream intake freeze result:
...
- aion_v0_1_0: ${tag_ref}
v0.2 workstream intake freeze PASS
SUMMARY
