...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-125 pre-implementation artifact: $file" >&2
    exit 1
  }
done
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-125 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-125 examples or static data" >&2
  exit 1
fi
...
print("v0.2 pre-implementation master no-go JSON checks PASS")
PY

cat <<'SUMMARY'
v0.2 pre-implementation master no-go regression result:
...
- migrations: absent
v0.2 pre-implementation master no-go regression PASS
SUMMARY
