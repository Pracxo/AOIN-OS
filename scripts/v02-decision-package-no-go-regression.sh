...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-138 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-138 examples or static data" >&2
  exit 1
fi

cat <<'SUMMARY'
v0.2 decision package no-go regression result:
...
- SDK or CLI implementation added: absent
v0.2 decision package no-go regression PASS
SUMMARY
