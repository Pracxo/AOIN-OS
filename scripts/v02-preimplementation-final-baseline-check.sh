...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_PREIMPLEMENTATION_BASELINE_SKIP_FULL_CHECK:-}" == "1" ]] && return 0
...
if is_nested_gate_context; then
  echo "PASS: v0.2 pre-implementation master freeze deferred to outer aggregate gate"
else
  AION_AGGREGATE_GATE_RUNNING=1 ./scripts/v02-preimplementation-master-freeze.sh
fi
...
echo "v0.2 pre-implementation final baseline working tree summary:"
...
cat <<SUMMARY
v0.2 pre-implementation final baseline result:
...
- aion_v0_1_0: ${tag_ref}
v0.2 pre-implementation final baseline PASS
SUMMARY
