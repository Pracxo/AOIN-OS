...
"$PYTHON_BIN" scripts/lib/cognitive_architecture_governance.py \
  --repo-root "$ROOT_DIR" \
  --mode memory-consolidation-no-go
...
cat <<'SUMMARY'
cognitive memory-consolidation no-go result:
...
- automatic_procedural_promotion=false
- hidden_memory_mutation=false
- source_rewrite=false
- deletion_without_policy_evidence=false
- network_calls=0
- connector_calls=0
...
- migrations=absent
cognitive memory-consolidation no-go PASS
SUMMARY
