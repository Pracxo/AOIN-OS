...
is_nested_gate_context() {
  ...
  [[ "${AION_PRODUCTION_AUTH_REQUEST_IDENTITY_RUNTIME_HOLD_SKIP_FULL_CHECK:-}" == "1" ]] && return 0
...
payload = json.loads(
    (root / "operator-console-static/demo-data/production-auth-request-identity-runtime-hold.json").read_text()
)
required_false = [
...
cat <<SUMMARY
production auth request identity runtime hold result:
...
- aion_v0_1_0: ${tag_ref}
production auth request identity runtime hold PASS
SUMMARY
