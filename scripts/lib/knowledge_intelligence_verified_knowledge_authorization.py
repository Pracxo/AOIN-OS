...
def validate_repository_state(root: Path) -> None:
    for relative in AION217_SOURCE_PATHS:
        if (root / relative).exists():
            raise ValueError(f"AION-217 source is not allowed on AION-216: {relative}")
