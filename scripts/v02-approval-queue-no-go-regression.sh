...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-127 approval queue no-go artifact: $file" >&2
    exit 1
  }
done
...
if changed_files | rg -n '^services/brain-api/src/aion_brain/api/'; then
  echo "AION-127 must not add or change API runtime execution routes" >&2
  exit 1
fi
...
cat <<'SUMMARY'
v0.2 approval queue no-go regression result:
- approval queue item approved true: absent
- proposal state implies implementation approved: absent
- proposal implementation approval true: absent
- implementation approval set true: absent
- workstream implementation approval set true: absent
- backlog implementation approval set true: absent
...
- approval expiry bypassed: absent
- approval revocation bypassed: absent
- dual-control bypassed: absent
...
v0.2 approval queue no-go regression PASS
SUMMARY
