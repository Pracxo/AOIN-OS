...
for file in "${required_artifacts[@]}"; do
  test -f "$file" || {
    echo "missing AION-119 planning artifact: $file" >&2
    exit 1
  }
done
...
if rg -n 'gh[[:space:]]+release[[:space:]]+create[[:space:]]+|release[[:space:]]+created[[:space:]]*[:=][[:space:]]*true|v02_release_created[[:space:]]*[:=][[:space:]]*true' \
  docs/release docs/adr/0110-v02-planning-charter.md examples/release operator-console-static/demo-data; then
  echo "v0.2 release creation or approval marker found" >&2
  exit 1
fi
...
if changed_files | rg -n '^services/brain-api/src/aion_brain/(connector_runtime|connector_simulator|connector_policy|connector_sandbox|connector_credentials|auth_runtime|local_auth|local_session|action_authorization|modules)/|^services/brain-api/src/aion_brain/config\.py$|^\.env\.example$'; then
  echo "AION-119 must not add or change runtime source or runtime config defaults" >&2
  exit 1
fi
...
if rg -n '\b(runtime_implementation_approved|operator_write_execution_approved|connector_implementation_approved|production_auth_approved|module_activation_approved|external_calls_approved|credential_storage_approved|token_storage_approved|sandbox_execution_approved|oauth_oidc_saml_runtime_approved|code_loading_approved|runtime_registration_approved|capability_activation_approved|v02_release_approved|v02_release_created|package_files_added|migrations_added|api_runtime_execution_route_added|sdk_resource_implementation_added|cli_command_implementation_added|frontend_dependencies_added)\s*[:=]\s*true\b' \
  "${scan_paths[@]}"; then
  echo "planning approval or drift boolean was enabled" >&2
  exit 1
fi

if rg -n '\b(v02_tag_created|v0_2_tag_created|v0\.2_tag_created)\s*[:=]\s*true\b|git[[:space:]]+tag[[:space:]]+(v0\.2|v0\.2\.0|aion-v0\.2\.0)' \
  docs/release docs/adr/0110-v02-planning-charter.md examples/release operator-console-static/demo-data; then
  echo "v0.2 tag creation or approval found" >&2
  exit 1
fi
...
if rg -n 'requests\.(get|post|put|patch|delete)|httpx\.(get|post|put|patch|delete)|aiohttp\.ClientSession|urllib\.request|socket\.|dns\.resolver' \
  ...
  echo "planning external call pattern found" >&2
  exit 1
fi
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-119 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  examples/release/v02-backlog-intake-result.json \
  operator-console-static/demo-data/v02-planning-charter.json \
  operator-console-static/demo-data/v02-gate-dependency-matrix.json; then
  echo "blocked marker found in AION-119 examples or static data" >&2
  exit 1
fi
...
print("v0.2 planning no-go JSON checks PASS")
PY

cat <<'SUMMARY'
v0.2 planning no-go regression result:
