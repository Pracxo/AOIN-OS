...
def main():
    ...
    assert len(active) == 1
    if pg['program_state'] in {'epistemic_truth_engine_authorized_not_implemented', 'epistemic_truth_engine_implemented_persistent_write_disabled_pending_closeout'}:
        assert pg['active_knowledge_implementation_authorization'] == 'AION-210-KI-0004'
        assert pg['active_knowledge_implementation_task'] == 'AION-211' and pg['formal_closeout_task'] == 'AION-212'
        ...
        r = active[0]
        assert r['authorization_transaction_id'] == 'AION-210-KI-0004'
        source = [x for x in au['records'] if x.get('authorization_transaction_id') == 'AION-206-KI-0002'][0]
        ...
        claim = [x for x in au['records'] if x.get('authorization_transaction_id') == 'AION-208-KI-0003'][0]
        ...
        assert pg['source_registry_persistent_write_enabled'] is False
        r = active[0]
        assert r['authorization_transaction_id'] == 'AION-218-KI-0008'
        ...
        assert closed['authorization_reusable'] is False
        assert closed['authorization_consumed_by_prs'] == [116, 117]
    elif pg['program_state'] in {'verified_knowledge_memory_authorized_not_implemented', 'verified_knowledge_memory_implemented_persistent_write_disabled_pending_closeout'}:
        ...
        assert pg['actual_tool_execution_enabled'] is False
        assert pg['verified_knowledge_memory_authorized'] is True
        assert pg['verified_knowledge_memory_implemented'] is (
            pg['program_state']
        ...
        if (
            ...
            assert pg['verified_knowledge_memory_state'] == (
                'implemented_deterministic_in_memory_candidate_versioning_engagement_learning_persistent_write_disabled'
            )
        assert pg['persistent_verified_knowledge_write_enabled'] is False
        assert pg['source_provenance_registry_implemented'] is True
        assert pg['source_provenance_registry_state'] == 'implemented_append_only_in_memory_replay_persistent_write_disabled'
        ...
        assert closed['authorization_reusable'] is False
        assert closed['authorization_consumed_by_prs'] == [116, 117]
    elif pg['program_state'] in {'tool_verification_fabric_authorized_not_implemented', 'tool_verification_fabric_implemented_persistent_write_disabled_pending_closeout'}:
        assert pg['active_knowledge_implementation_authorization'] == 'AION-214-KI-0006'
        assert pg['active_knowledge_implementation_task'] == 'AION-215' and pg['formal_closeout_task'] == 'AION-216'
        assert pg['domain_expert_mesh_implemented'] is True
        assert pg['model_call_enabled'] is False
        assert pg['persistent_mesh_write_enabled'] is False
        assert pg['tool_verification_fabric_authorized'] is True
        assert pg['tool_verification_fabric_implemented'] is (
            pg['program_state']
        ...
        if (
            ...
            assert pg['tool_verification_fabric_state'] == (
        ...
        assert pg['source_registry_persistent_write_enabled'] is False
        r = active[0]
        assert r['authorization_transaction_id'] == 'AION-214-KI-0006'
        for expected_id, closed_by in {
            'AION-206-KI-0002': 'AION-208',
            'AION-208-KI-0003': 'AION-210',
            'AION-210-KI-0004': 'AION-212',
        ...
        assert closed['authorization_reusable'] is False
        assert closed['authorization_consumed_by_prs'] == [116, 117]
    elif pg['program_state'] in {'domain_expert_mesh_authorized_not_implemented', 'domain_expert_mesh_implemented_persistent_write_disabled_pending_closeout'}:
        assert pg['active_knowledge_implementation_authorization'] == 'AION-212-KI-0005'
        assert pg['active_knowledge_implementation_task'] == 'AION-213' and pg['formal_closeout_task'] == 'AION-214'
        if pg['program_state'] == 'domain_expert_mesh_implemented_persistent_write_disabled_pending_closeout':
            assert pg['domain_expert_mesh_implemented'] is True
            assert pg['model_call_enabled'] is False
            assert pg['persistent_mesh_write_enabled'] is False
        assert pg['source_provenance_registry_implemented'] is True
        ...
        assert pg['source_registry_persistent_write_enabled'] is False
        r = active[0]
        assert r['authorization_transaction_id'] == 'AION-212-KI-0005'
        source = [x for x in au['records'] if x.get('authorization_transaction_id') == 'AION-206-KI-0002'][0]
        assert source['authorization_active'] is False
        assert source['authorization_consumed'] is True
        assert source['authorization_expired'] is True
        ...
        assert claim['authorization_reusable'] is False
        assert claim['authorization_closed_by_task'] == 'AION-210'
        epistemic = [x for x in au['records'] if x.get('authorization_transaction_id') == 'AION-210-KI-0004'][0]
        assert epistemic['authorization_active'] is False
        assert epistemic['authorization_consumed'] is True
        assert epistemic['authorization_expired'] is True
        assert epistemic['authorization_reusable'] is False
        assert epistemic['authorization_closed_by_task'] == 'AION-212'
        closed = [x for x in au['records'] if x.get('authorization_transaction_id') == AUTH_ID][0]
        assert closed['authorization_active'] is False
        assert closed['authorization_consumed'] is True
        assert closed['authorization_expired'] is True
        assert closed['authorization_reusable'] is False
        assert closed['authorization_consumed_by_prs'] == [116, 117]
    elif pg['program_state'] in {'temporal_claim_evidence_graph_authorized_not_implemented', 'temporal_claim_evidence_graph_implemented_write_disabled_pending_closeout'}:
        ...
        assert pg['source_registry_persistent_write_enabled'] is False
        r = active[0]
        assert r['authorization_transaction_id'] == 'AION-208-KI-0003'
        source = [x for x in au['records'] if x.get('authorization_transaction_id') == 'AION-206-KI-0002'][0]
        assert source['authorization_active'] is False
        assert source['authorization_consumed'] is True
        assert source['authorization_expired'] is True
        ...
        assert closed['authorization_reusable'] is False
        assert closed['authorization_consumed_by_prs'] == [116, 117]
    elif pg['program_state'] in {'source_provenance_registry_authorized_not_implemented', 'source_provenance_registry_implemented_write_disabled_pending_closeout'}:
        ...
        if pg['program_state'] == 'source_provenance_registry_implemented_write_disabled_pending_closeout':
            assert pg['source_provenance_registry_implemented'] is True
            assert pg['source_provenance_registry_state'] == 'implemented_append_only_in_memory_replay_persistent_write_disabled'
            assert pg['source_registry_runtime_enabled'] is False
            assert pg['source_registry_persistent_write_enabled'] is False
    ...
    if pg['research_plane_implemented'] is True:
        ...
        assert pg['research_plane_state'] == 'implemented_operator_invoked_disabled'
        assert pg['public_network_fetch_available'] is False
    else:
