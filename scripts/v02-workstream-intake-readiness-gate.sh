...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing v0.2 workstream intake artifact: $file" >&2
    exit 1
  }
done

grep -q "0115-v02-workstream-intake-readiness.md" docs/adr/README.md || {
  echo "ADR 0115 is not indexed" >&2
  exit 1
}
...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_WORKSTREAM_INTAKE_SKIP_NESTED_GATES:-}" == "1" ]] && return 0
...
print("v0.2 workstream intake JSON checks PASS")
PY

cat <<'SUMMARY'
v0.2 workstream intake readiness gate result:
- v02_workstream_intake_ready: true
...
v0.2 workstream intake readiness gate PASS
SUMMARY
