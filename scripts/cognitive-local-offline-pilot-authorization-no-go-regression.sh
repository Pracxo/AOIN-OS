...
"$PYTHON_BIN" scripts/lib/cognitive_architecture_governance.py \
  --repo-root "$ROOT_DIR" \
  --mode local-offline-pilot-authorization-no-go
...
cat <<'SUMMARY'
cognitive local-offline pilot authorization no-go result:
- pilot_executed=false
- aion202_pilot_evidence=absent
...
- migrations=absent
cognitive local-offline pilot authorization no-go PASS
SUMMARY
