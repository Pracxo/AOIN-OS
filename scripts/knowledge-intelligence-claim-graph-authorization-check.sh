...
POST_AION216_STATE = "verified_knowledge_memory_authorized_not_implemented"
...
if program["program_state"] in SUCCESSOR_STATES:
    if program["program_state"] in {POST_AION216_STATE, POST_AION217_IMPLEMENTED_STATE}:
        assert active[0]["authorization_transaction_id"] == "AION-216-KI-0007"
    elif program["program_state"] in {POST_AION214_STATE, POST_AION215_IMPLEMENTED_STATE}:
...
if program["program_state"] in SUCCESSOR_STATES:
    if program["program_state"] in {POST_AION216_STATE, POST_AION217_IMPLEMENTED_STATE}:
        ...
        assert program["verified_knowledge_memory_implemented"] is (
            program["program_state"] == POST_AION217_IMPLEMENTED_STATE
        )
        ...
        if program["program_state"] == POST_AION213_STATE:
            assert program["domain_expert_mesh_implemented"] is True
            assert program["model_call_enabled"] is False
            assert program["persistent_mesh_write_enabled"] is False
    else:
...
for key in FALSE_FLAGS:
    if (
        key == "new_knowledge_implementation_authorization_created"
        and program["program_state"] in {
            POST_AION210_STATE,
            POST_AION212_STATE,
            POST_AION213_STATE,
            POST_AION214_STATE,
