...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-128 planning master no-go artifact: $file" >&2
    exit 1
  }
done
...
cat <<'SUMMARY'
v0.2 planning master no-go regression result:
- v0.2 tag created: absent
- v0.2 release created: absent
- runtime implementation approval true: absent
- backlog implementation approval true: absent
...
v0.2 planning master no-go regression PASS
SUMMARY
