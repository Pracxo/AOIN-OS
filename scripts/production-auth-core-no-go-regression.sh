...
while IFS= read -r file; do
  ...
  if aion168_is_scoped_self_improvement_evaluation_plane_path "$file"; then
    continue
  fi
  if aion169_is_scoped_self_improvement_experiment_authorization_path "$file"; then
    continue
  fi
  if aion171_is_scoped_self_improvement_rewrite_authorization_path "$file"; then
    continue
  fi
  if aion172_is_scoped_self_improvement_rewrite_controller_path "$file"; then
    continue
  fi
  if aion173_is_scoped_self_improvement_canary_authorization_path "$file"; then
    continue
  fi
  ...
  if aion178_is_scoped_self_improvement_shadow_mode_path "$file"; then
    continue
  fi
  if aion180_is_scoped_self_improvement_shadow_activation_authorization_path "$file"; then
    continue
  fi
  case "$file" in
