...
is_nested_gate_context() {
  ...
  [[ "${AION_CONNECTOR_PLATFORM_FREEZE_SKIP_FULL_CHECK:-}" == "1" ]] && return 0
  [[ "${AION_AGGREGATE_GATE_RUNNING:-}" == "1" ]] && return 0
  [[ "${AION_CHECK_RUNNING:-}" == "1" ]] && return 0
  return 1
}

if [[ "${AION_CONNECTOR_PLATFORM_FREEZE_SKIP_CHECKPOINT:-}" == "1" ]] || is_nested_gate_context; then
...
echo "Connector platform freeze working tree summary:"
...
cat <<SUMMARY
Connector platform freeze result:
...
- aion_v0_1_0: ${tag_ref}
Connector platform freeze PASS
SUMMARY
