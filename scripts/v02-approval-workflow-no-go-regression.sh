...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-123 approval workflow artifact: $file" >&2
    exit 1
  }
done
...
if rg -n 'gh[[:space:]]+release[[:space:]]+create[[:space:]]+|release[[:space:]]+created[[:space:]]*[:=][[:space:]]*true|v02_release_created[[:space:]]*[:=][[:space:]]*true' \
  docs/release docs/adr/0114-v02-approval-workflow-stabilization.md examples/release operator-console-static/demo-data; then
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-123 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-123 examples or static data" >&2
  exit 1
fi
...
print("v0.2 approval workflow no-go JSON checks PASS")
PY

cat <<'SUMMARY'
v0.2 approval workflow no-go regression result:
...
- migrations: absent
v0.2 approval workflow no-go regression PASS
SUMMARY
