...
"$PYTHON_BIN" scripts/lib/cognitive_architecture_governance.py \
  --repo-root "$ROOT_DIR" \
  --mode information-acquisition-closeout-no-go
...
cat <<'SUMMARY'
cognitive information-acquisition closeout no-go result:
...
- migrations=absent
cognitive information-acquisition closeout no-go PASS
SUMMARY
