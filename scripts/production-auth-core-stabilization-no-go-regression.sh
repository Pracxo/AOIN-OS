...
while IFS= read -r file; do
  ...
  case "$file" in
    ...
    */package.json|*/package-lock.json|*/pnpm-lock.yaml|*/yarn.lock|*/bun.lockb)
      echo "package files and lockfiles are forbidden for AION-154: $file" >&2
      exit 1
      ;;
    packages/aion-sdk-python/src/*)
      echo "SDK or CLI runtime source is forbidden for AION-154: $file" >&2
      exit 1
      ;;
    *migrations/*|*alembic/*|*migration*.py|*migration*.sql)
      echo "migrations are forbidden for AION-154: $file" >&2
      exit 1
      ;;
  ...
  if ! aion154_is_scoped_stabilization_path "$file" \
    ...
    case "$file" in
      ...
      services/brain-api/src/aion_brain/kernel/diagnostics.py)
        echo "unapproved production-auth source path for AION-154: $file" >&2
        exit 1
        ;;
    esac
  fi
...
if [[ -s "$scan_file_list" ]]; then
  if rg -n '@router\.(get|post|put|patch|delete)\([^)]*(production-auth|auth/production|login|logout|callback|oauth|oidc|saml|token|session|credentials)' $(cat "$scan_file_list"); then
    echo "runtime auth route surface found in changed files" >&2
    exit 1
  fi

  if rg -n 'requests\.(get|post|put|patch|delete)|httpx\.(get|post|put|patch|delete)|aiohttp\.ClientSession|urllib\.request|socket\.|dns\.resolver' $(cat "$scan_file_list"); then
...
echo "production auth core stabilization no-go PASS"
