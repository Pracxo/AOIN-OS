...
hold = json.loads((root / "examples/secure-runtime-integration/operator-console-runtime-hold.json").read_text())
for key in (
...
if hold["loopback_listener_absent"] is not True:
    raise SystemExit("loopback listener must be absent in AION-236")
PY
