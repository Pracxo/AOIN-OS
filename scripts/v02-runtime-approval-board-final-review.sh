...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_RUNTIME_APPROVAL_BOARD_FINAL_REVIEW_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-146 runtime approval board final artifact: $file" >&2
    exit 1
  }
done

grep -q "0137-v02-runtime-approval-board-final-review.md" docs/adr/README.md || {
  echo "ADR 0137 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 runtime approval board final inherited gates deferred to outer aggregate gate"
else
...
if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-146" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-runtime-approval-board-final-review.json",
    root / "examples/release/v02-approval-vote-record-closeout.json",
    root / "examples/release/v02-implementation-go-no-go-ledger-final-lock.json",
    root / "examples/release/v02-runtime-approval-board-final-evidence-matrix.json",
    root / "examples/release/v02-final-implementation-go-guard.json",
    root / "operator-console-static/demo-data/v02-runtime-approval-board-final-review.json",
    root / "operator-console-static/demo-data/v02-implementation-go-no-go-ledger-final-lock.json",
]
true_keys = {
...
def walk_false(value: Any, context: str) -> None:
    if isinstance(value, dict):
        for key, nested in value.items():
            ...
            if key in {
                ...
                assert nested is False, f"{context}.{key} must be false"
            walk_false(nested, f"{context}.{key}")
    elif isinstance(value, list):
        for index, item in enumerate(value):
            walk_false(item, f"{context}[{index}]")


for path in paths:
    payload = json.loads(path.read_text())
    for key in true_keys:
        assert payload.get(key) is True, f"{path.name}: {key} must be true"
    for key in false_keys:
        assert payload.get(key) is False, f"{path.name}: {key} must be false"
    assert required_safety.issubset(set(payload.get("content_safety", []))), path.name
    actions = {
    ...
    assert forbidden_actions.issubset(actions), f"{path.name}: forbidden actions missing"
    walk_false(payload, path.name)
PY

cat <<'SUMMARY'
v0.2 runtime approval board final review result:
