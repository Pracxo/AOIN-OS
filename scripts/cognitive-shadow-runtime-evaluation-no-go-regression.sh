...
"$PYTHON_BIN" scripts/lib/cognitive_architecture_governance.py \
  --repo-root "$ROOT_DIR" \
  --mode shadow-runtime-evaluation-no-go
...
cat <<'SUMMARY'
cognitive shadow-runtime evaluation no-go result:
...
- pilot_authorization_created=false
- package_files=absent
- migrations=absent
cognitive shadow-runtime evaluation no-go PASS
SUMMARY
