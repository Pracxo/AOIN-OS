...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-144 runtime approval board artifact: $file" >&2
    exit 1
  }
done
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-144 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-144 examples or static data" >&2
  exit 1
fi

cat <<'SUMMARY'
v0.2 runtime approval board no-go regression result:
...
- go/no-go ledger runtime effect true: absent
- approval docket final review approval true: absent
- approval docket item approved true: absent
- approval docket stabilization approval true: absent
- implementation decision record approval true: absent
- implementation decision record closeout approval true: absent
- runtime approval lock release approved true: absent
- runtime approval review approved true: absent
- runtime decision lock release approved true: absent
- runtime decision readiness approved true: absent
- decision package approval true: absent
- approval readiness approved true: absent
- review board decision approval true: absent
- routing decision approval true: absent
- reviewer signoff implementation approval true: absent
- preapproval queue item approved true: absent
- submission approval true: absent
- request pack approval true: absent
- request package implementation approved true: absent
- proposal template implementation approved true: absent
