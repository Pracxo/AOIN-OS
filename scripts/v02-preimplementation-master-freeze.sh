...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing v0.2 pre-implementation master freeze artifact: $file" >&2
    exit 1
  }
done

grep -q "0116-v02-preimplementation-master-freeze.md" docs/adr/README.md || {
  echo "ADR 0116 is not indexed" >&2
  exit 1
}

if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-125" >&2
  exit 1
fi

is_nested_gate_context() {
  [[ -n "${PYTEST_CURRENT_TEST:-}" ]] && return 0
  [[ "${AION_V02_PREIMPLEMENTATION_MASTER_SKIP_NESTED_GATES:-}" == "1" ]] && return 0
...
if is_nested_gate_context; then
  echo "PASS: v0.2 pre-implementation downstream release gates deferred to outer aggregate gate"
else
  AION_V02_WORKSTREAM_INTAKE_SKIP_NESTED_GATES=1 ./scripts/v02-workstream-intake-readiness-gate.sh
  AION_V02_WORKSTREAM_INTAKE_FREEZE_SKIP_FULL_CHECK=1 ./scripts/v02-workstream-intake-freeze.sh
...
paths = [
    root / "examples/release/v02-preimplementation-master-freeze.json",
    root / "examples/release/v02-final-planning-baseline.json",
    root / "examples/release/v02-workstream-governance-closeout.json",
    root / "examples/release/v02-approval-workflow-closeout-evidence.json",
    root / "examples/release/v02-implementation-lock-summary.json",
    root / "operator-console-static/demo-data/v02-preimplementation-master-freeze.json",
    root / "operator-console-static/demo-data/v02-final-planning-baseline.json",
]
false_keys = {
...
for path in paths:
    ...
    if relative.startswith("examples/") and payload.get("synthetic") is not True:
        raise SystemExit(f"pre-implementation example must be synthetic: {path}")
    if payload.get("status") != "passed":
        raise SystemExit(f"pre-implementation artifact must be passed: {path}")
    if payload.get("task_id") != "AION-125":
        raise SystemExit(f"pre-implementation task id must be AION-125: {path}")
    if payload.get("v02_preimplementation_master_freeze_passed") is not True:
        raise SystemExit(f"master freeze flag must be true: {path}")
    walk(payload, relative)

print("v0.2 pre-implementation master JSON checks PASS")
PY

cat <<'SUMMARY'
v0.2 pre-implementation master freeze result:
- v02_preimplementation_master_freeze_passed: true
...
v0.2 pre-implementation master freeze PASS
SUMMARY
