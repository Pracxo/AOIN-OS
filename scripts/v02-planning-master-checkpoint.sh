...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_PLANNING_MASTER_CHECKPOINT_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
for file in "${required_docs[@]}" "${required_examples[@]}"; do
  test -f "$file" || {
    echo "missing AION-128 planning master artifact: $file" >&2
    exit 1
  }
done

grep -q "0119-v02-planning-master-checkpoint.md" docs/adr/README.md || {
  echo "ADR 0119 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 planning master inherited gates deferred to outer aggregate gate"
else
...
if git tag --list | rg -n '^(v0\.2|v0\.2\.0|aion-v0\.2\.0|aion-v0\.2)$'; then
  echo "v0.2 tag must not exist for AION-128" >&2
  exit 1
fi
...
if rg -n 'npm[[:space:]]+install|pnpm[[:space:]]+install|yarn[[:space:]]+add|bun[[:space:]]+add|pip[[:space:]]+install' \
  ...
  echo "package install instruction found in AION-128 artifacts" >&2
  exit 1
fi
...
if rg -n 'https?://|sk-|ghp_|xoxb-|-----BEGIN PRIVATE KEY-----|bearer |basic |api_key|private_key|access_token|refresh_token|id_token|client_secret|raw_prompt|hidden_reasoning|chain_of_thought' \
  ...
  echo "blocked marker found in AION-128 examples or static data" >&2
  exit 1
fi
...
paths = [
    root / "examples/release/v02-planning-master-checkpoint.json",
    root / "examples/release/v02-proposal-governance-baseline.json",
    root / "examples/release/v02-implementation-lock-freeze.json",
    root / "examples/release/v02-approval-queue-baseline-summary.json",
    root / "examples/release/v02-planning-master-evidence-matrix.json",
    root / "operator-console-static/demo-data/v02-planning-master-checkpoint.json",
    root / "operator-console-static/demo-data/v02-implementation-lock-freeze.json",
]
false_keys = {
    "v02_tag_created",
    "v02_release_created",
...
for path in paths:
    ...
    if payload.get("task_id") != "AION-128":
    ...
    if payload.get("v02_planning_master_checkpoint_passed") is not True:
        raise SystemExit(f"{path} must mark planning master passed")
...
cat <<'SUMMARY'
v0.2 planning master checkpoint result:
- v02_planning_master_checkpoint_passed: true
...
v0.2 planning master checkpoint PASS
SUMMARY
