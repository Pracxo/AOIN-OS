...
is_nested_gate_context() {
  ...
  [[ "${AION_V02_RUNTIME_APPROVAL_BOARD_PREVIEW_SKIP_INHERITED_GATES:-}" == "1" ]] && return 0
...
grep -q "0135-v02-runtime-approval-board-preview.md" docs/adr/README.md || {
  echo "ADR 0135 is not indexed" >&2
  exit 1
}

if is_nested_gate_context; then
  echo "PASS: v0.2 runtime approval board inherited gates deferred to outer aggregate gate"
else
...
paths = [
    root / "examples/release/v02-runtime-approval-board-preview.json",
    root / "examples/release/v02-approval-vote-record-guard.json",
    root / "examples/release/v02-implementation-go-no-go-ledger-boundary.json",
    root / "examples/release/v02-runtime-approval-board-state-model.json",
    root / "examples/release/v02-runtime-approval-board-evidence-pack.json",
    root / "operator-console-static/demo-data/v02-runtime-approval-board-preview.json",
    root / "operator-console-static/demo-data/v02-implementation-go-no-go-ledger-boundary.json",
]
true_keys = {
...
for path in paths:
    ...
    assert payload["task_id"] == "AION-144", context
...
cat <<'SUMMARY'
v0.2 runtime approval board preview check result:
- docs exist: true
- ADR 0135 indexed: true
